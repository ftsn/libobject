...
#define Iterator_vtable_definition  \
        .equals = it_equals,		\
        .previous = NULL,			\
        .next = NULL,				\
        .dereference = NULL
_forward_declared_class_declaration(Iterator)

// Forward iterators
#define ForwardIterator_fields		        \
        Iterator_fields
#define ForwardIterator_vtable_fields       \
        Iterator_vtable_fields
#define ForwardIterator_definition	        \
        Iterator_definition
#define ForwardIterator_vtable_definition   \
        Iterator_vtable_definition
_forward_declared_class_declaration(ForwardIterator)

#define SplListFwdIterator_fields			    \
        ForwardIterator_fields
#define SplListFwdIterator_vtable_fields        \
        ForwardIterator_vtable_fields
#define SplListFwdIterator_definition		    \
        ForwardIterator_definition
#define SplListFwdIterator_vtable_definition    \
        ForwardIterator_vtable_definition,      \
        .next = list_it_next,				    \
		.dereference = list_it_dereference
_class_declaration(SplListFwdIterator)

// BiDirectional iterators
#define BidirectionalIterator_fields    	    \
        Iterator_fields
#define BidirectionalIterator_vtable_fields     \
        Iterator_vtable_fields
#define BidirectionalIterator_definition        \
        Iterator_definition
#define BidirectionalIterator_vtable_definition \
        Iterator_vtable_definition
_forward_declared_class_declaration(BidirectionalIterator)

#define DblListBidirectionalIterator_fields 	        \
        BidirectionalIterator_fields
#define DblListBidirectionalIterator_vtable_fields      \
        BidirectionalIterator_vtable_fields
#define DblListBidirectionalIterator_definition	        \
        BidirectionalIterator_definition
#define DblListBidirectionalIterator_vtable_definition  \
        BidirectionalIterator_vtable_definition,        \
        .previous = list_it_previous,			        \
		.next = list_it_next,					        \
		.dereference = list_it_dereference

_class_declaration(DblListBidirectionalIterator)

#define DictBidirectionalIterator_fields                    \
        BidirectionalIterator_fields                        \
        ssize_t internal_idx;
#define DictBidirectionalIterator_vtable_fields             \
        BidirectionalIterator_vtable_fields
#define DictBidirectionalIterator_definition                \
        BidirectionalIterator_definition,                   \
		.internal_idx = 0
#define DictBidirectionalIterator_vtable_definition         \
        BidirectionalIterator_vtable_definition,            \
        .previous = dict_bidirectional_it_previous,         \
		.next = dict_bidirectional_it_next,                 \
		.dereference = dict_bidirectional_it_dereference
_class_declaration(DictBidirectionalIterator)

// Random access iterators
#define RandomAccessIterator_fields \
        Iterator_fields             \
        ssize_t             ra_idx;
#define RandomAccessIterator_vtable_fields  \
        Iterator_vtable_fields              \
        t_ra_it_compare     lt;             \
        t_ra_it_compare     gt;             \
        t_ra_it_move        jump;           \
...
#define RandomAccessIterator_definition		\
        Iterator_definition,				\
		.ra_idx = 0
#define RandomAccessIterator_vtable_definition  \
        Iterator_vtable_definition,             \
        .dereference = ra_it_dereference,       \
		.lt = ra_it_lt,                         \
		.gt = ra_it_gt
_forward_declared_class_declaration(RandomAccessIterator)

#define ArrayRaIterator_fields				    \
        RandomAccessIterator_fields
#define ArrayRaIterator_vtable_fields           \
        RandomAccessIterator_vtable_fields
#define ArrayRaIterator_definition			    \
        RandomAccessIterator_definition
#define ArrayRaIterator_vtable_definition       \
        RandomAccessIterator_vtable_definition, \
        .previous = array_ra_it_previous,	    \
		.next = array_ra_it_next,			    \
		.jump = array_ra_it_jump,			    \
		.at = array_ra_it_at

_class_declaration(ArrayRaIterator)

#define StringRaIterator_fields				    \
        RandomAccessIterator_fields
#define StringRaIterator_vtable_fields          \
        RandomAccessIterator_vtable_fields
#define StringRaIterator_definition			    \
        RandomAccessIterator_definition
#define StringRaIterator_vtable_definition      \
        RandomAccessIterator_vtable_definition, \
        .previous = string_ra_it_previous,	    \
		.next = string_ra_it_next,			    \
		.jump = string_ra_it_jump,			    \
		.at = string_ra_it_at
_class_declaration(StringRaIterator)
